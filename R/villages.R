...
#'
#' @md
#' @param village_ids a character vector of village ids
...
#' @return data frame
#' @export
ial_villages <- function(village_ids, interval_start, interval_stop) {

  village_ids <- paste0(village_ids, collapse=",")
  ...
  url_template <- "http://api.nightlights.io/months/%s-%s/villages/%s"

  httr::GET(
    url = sprintf(url_template, interval_start, interval_stop, village_ids),
